...
against a pydantic model (`cli/validator.py:272`). Both are hand-written.
"""
...
ROUTER_HONOURED_KEYS = [
    ...
    (RouterReplayPluginConfig, "max_tool_trace_bytes", "plugin_config.go:265"),
    (RouterReplayPluginConfig, "max_tool_trace_steps", "plugin_config.go:272"),
]
...
def test_plugin_validator_rejects_unknown_keys():
    ...
    `cli/models.py` only `ContextCompressionPluginConfig:957` and
    `ShadowDispatchPluginConfig:1220` forbid extras.
    """
    accepted = [
        plugin_type

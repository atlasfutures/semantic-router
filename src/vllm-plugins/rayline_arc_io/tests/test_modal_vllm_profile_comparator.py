...
from modal_vllm_profile_comparator import benchmark_turn_states  # noqa: E402
...
def test_perf028_history_states_are_strict_public_turn_prefixes() -> None:
...
def test_perf028_contract_is_bounded_and_profiles_only_one_backend_axis() -> None:
    receipt = budget_receipt(PERF028_BUDGET)
    ...
    assert receipt["maximum_resource_envelope_usd"] < PERF028_BUDGET.packet_ceiling_usd
...
def test_perf028_full_comparator_accepts_a_faster_parity_candidate(
    monkeypatch: Any,
) -> None:
    ...
    report = comparator.run_comparison(clients, "test-perf028")

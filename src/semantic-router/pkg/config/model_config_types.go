...
type ModelParams struct {
	...
	// Disabled takes the model out of service without removing its card. A
	// fixed, ordinal-indexed arm set cannot delete an arm that must stop
	// serving, because the artifact head, the exclusion mask and the model
...
// nothing, which makes a routing-capability gate fail closed onto a replayable
// status rather than route a turn to an arm that answers it on less than the
// caller sent. The routing names are the RoutingCapability constants in
// pkg/llmprotocol.

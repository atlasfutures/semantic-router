...
func validateDecisionModelContracts(cfg *RouterConfig) error {
	for _, decision := range cfg.AllRoutingDecisions() {
		if err := validateDecisionModelContract(cfg, decision); err != nil {
			return err
		}
	}
	return nil
...
func validateDecisionModelContract(cfg *RouterConfig, decision Decision) error {
...
func configuredAlgorithmBlocks(algorithm *AlgorithmConfig) []string {
	configuredBlocks := make([]string, 0, 17)

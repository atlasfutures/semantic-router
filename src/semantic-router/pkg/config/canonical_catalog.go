...
func appendMaterializedEndpoint(
	...
	cfg.VLLMEndpoints = append(cfg.VLLMEndpoints, VLLMEndpoint{
		...
		// Credential provenance travels with the endpoint so a readiness
		// check can prove which environment variable supplied the key.
		APIKeyEnvName: provider.Instance.Credentials.APIKeyEnv,
		APIKeyInline:  provider.Instance.Credentials.APIKey != "",
	})

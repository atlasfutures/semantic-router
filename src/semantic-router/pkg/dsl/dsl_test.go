...
// unmarshalFlatRouterConfigForTest decodes the flat emitter output back into a
// RouterConfig. RouterConfig has no flat decisions field anymore — config.Recipes
// owns them — so the emitted `decisions:` key is decoded separately and
// normalized into the default recipe, the way the loaders normalize a routing
// profile.
func unmarshalFlatRouterConfigForTest(data []byte, cfg *config.RouterConfig) error {
...
`
	...
	if err := unmarshalFlatRouterConfigForTest(yamlBytes, &roundTripped); err != nil {
...
`
	...
	if err := unmarshalFlatRouterConfigForTest(yamlBytes, &rt); err != nil {
		t.Fatalf("yaml.Unmarshal failed: %v\nYAML:\n%s", err, string(yamlBytes))
	}
...
func TestDoubleRoundTrip(t *testing.T) {
	...
	if unmarshalErr := unmarshalFlatRouterConfigForTest(yaml1, &rt1); unmarshalErr != nil {
		t.Fatalf("unmarshal1 error: %v", unmarshalErr)
	}

	// Second round-trip: marshal rt1 back to flat YAML. Decisions live on the
	// default recipe, so the flat emitter — not a bare yaml.Marshal — is what
	// re-emits the `decisions:` key.
	yaml2, err := marshalFlatConfigYAML(&rt1)
	if err != nil {
		t.Fatalf("marshal2 error: %v", err)
	}
	var rt2 config.RouterConfig
	if err := unmarshalFlatRouterConfigForTest(yaml2, &rt2); err != nil {

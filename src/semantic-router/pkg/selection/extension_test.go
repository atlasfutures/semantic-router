...
// The eval selection receipt hashes a JSON-encoded SelectionResult. The bag
// must stay out of that encoding, including a value JSON cannot encode.
func TestExtensionsAreNotSerialised(t *testing.T) {
	result := &SelectionResult{SelectedModel: "model-a"}
	result.SetExtension(extTraceKey, extTrace{SelectedArm: 2})
	result.SetExtension(unusedKey, func() {})
	selCtx := &SelectionContext{}
	selCtx.SetExtension(extInputKey, &extInput{EpisodeID: "episode-1"})

	for name, value := range map[string]any{"result": result, "context": selCtx} {
		raw, err := json.Marshal(value)
		if err != nil {
			t.Fatalf("%s: encoding failed because of the bag: %v", name, err)
		}
		if strings.Contains(string(raw), "Extensions") || strings.Contains(string(raw), "episode-1") {
			t.Fatalf("%s: bag reached the encoding: %s", name, raw)
		}
	}
}

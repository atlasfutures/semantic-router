...
func assertDefaultAliasStaysIsolated(ctx context.Context, localPort string, verbose bool) (string, error) {
	...
	decision := response.Headers.Get("x-vsr-selected-decision")
	if decision == "" {
		return "", fmt.Errorf("default-recipe alias %s returned an empty x-vsr-selected-decision header; the default profile must still route alias traffic through its own decisions",
			entrypointDefaultAlias)
	}
	if decision == entrypointProbeDecision {

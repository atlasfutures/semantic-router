...
"""Run the AGT013 agentic workload against the native Modal Rayline router."""
...
def _native_phase_report(phase: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "end_to_end_latency": _summary([result["total_seconds"] for result in results]),
        "decision_latency": _summary(
            [result["decision_latency_seconds"] for result in results]
        ),
        "embedding_latency": _summary(
            [result["embedding_latency_seconds"] for result in results]
        ),
        "q_latency": _summary([result["q_latency_seconds"] for result in results]),
